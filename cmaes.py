...
def runner(trial_id, eps_id, params, return_dict):
    ...
    delay = env.close()
    return_dict[trial_id+eps_id] = delay    # -cumulative_reward
...
class Agent(object):
    ...
    def act(self, state, reward, done):
        ...
        for i in range(len(self.env.actions)):
            ...
            modifs = 0
            for j in range(NUM_MEASUREMENTS-4, NUM_MEASUREMENTS):
                modifs += demand[i * NUM_MEASUREMENTS + j]
            action_values.append(modifs * sum)
